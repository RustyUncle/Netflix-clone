...
#[get("/movies")] 
async fn movies(tmpl: web::Data<Tera>, session: Session, db: web::Data<PgPool>) -> Result<HttpResponse, Error> {
    ...
    let query = sqlx::query_as::<_,Entity>("SELECT DISTINCT(entities.id), entities.name, entities.preview, entities,thumbnail,
        entities.categoryid FROM entities INNER JOIN videos ON entities.id = videos.entityid WHERE videos.isMovie = 1")
        .fetch_all(&**db)
        .await;
    ...
    let body = tmpl.render("movies.html", &ctx).map_err(error::ErrorInternalServerError)?;
